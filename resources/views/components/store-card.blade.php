...
<div class="bg-surface-container-lowest rounded-3xl p-6 border border-outline-variant hover:shadow-xl transition-shadow flex flex-col items-center text-center gap-4">
    <img src="{{ $store->logo }}" alt="{{ $store->name }} logo" class="w-20 h-20 rounded-full object-cover shadow-md">
    ...
    <a
        href="{{ $store->url ?? '#' }}"
        class="mt-2 px-5 py-2 rounded-full border-2 border-primary text-primary font-label-md hover:bg-primary/5 transition-colors">
        Visit Store
    </a>
</div>
